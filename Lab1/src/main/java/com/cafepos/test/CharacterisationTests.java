...
public class CharacterisationTests
{
    ...
    {
        String receipt = OrderManagerGod.process("espresso SHOT OAT", 1, "CASH", "NONE", false);
        assertTrue(receipt.startsWith("Order (espresso SHOT OAT) x1"));
    ...
    {
        String receipt = OrderManagerGod.process("latte L", 2, "CARD","LOYAL5", false);
        assertTrue(receipt.contains("Subtotal: 9.20"));
        assertTrue(receipt.contains("Discount: -0.46"));
        assertTrue(receipt.contains("Tax (10%): 0.87"));
        assertTrue(receipt.contains("Total: 9.61"));
    }

    @Test
    void coupon_fixed_amount_and_qty_clamp()
    {
        String receipt = OrderManagerGod.process("espresso SHOT", 0, "WALLET","COUPON1", false);
        assertTrue(receipt.contains("Order (espresso SHOT) x1"));
